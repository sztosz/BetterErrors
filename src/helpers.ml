let listDrop n lst =
  let lst = ref lst in for i = 1 to n do lst := (List.tl (!lst)) done; !lst
let listDropWhile f lst =
  let lst = ref lst in
  while f (List.hd (!lst)) do lst := (List.tl (!lst)) done; !lst
let listTake n lst =
  let result = ref [] in
  let lst = ref lst in
  for i = 1 to n do
    (result := ((List.hd (!lst)) :: (!result)); lst := (List.tl (!lst)))
  done;
  List.rev (!result)
let listTakeWhile f lst =
  ...
  while f (List.hd (!lst)) do
    (result := ((List.hd (!lst)) :: (!result)); lst := (List.tl (!lst))) done;
  List.rev (!result)
let optionGet a =
  match a with
  | ((Some (n))[@explicit_arity ]) -> n
  | None  -> raise ((Invalid_argument ("optionGet"))[@explicit_arity ])
let optionMap f a =
  match a with
  | ((Some (a'))[@explicit_arity ]) -> Some (f a')
  | None  -> None
let listFilterMap f lst =
  ((List.map f lst) |>
     (List.filter
        (function | ((Some (a))[@explicit_arity ]) -> true | None  -> false)))
    |> (List.map optionGet)
...
let stringSlice ?(first= 0)  ?last  str =
  let last =
    match last with
    | ((Some (l))[@explicit_arity ]) -> min l (String.length str)
    | None  -> String.length str in
  if last <= first then "" else String.sub str first (last - first)
let stringFind str part =
  let rec find' str part idx =
    if (String.length str) < (String.length part)
    then raise Not_found
    else
      if (stringSlice str ~last:(String.length part)) = part
      then idx
      else find' (stringSlice str ~first:1) part (idx + 1) in
  find' str part 0
let stringNsplit str ~by  =
  if (String.length str) = 0
  then
    raise
      ((Invalid_argument ("stringNSplit: empty str not allowed"))[@explicit_arity
                                                                   ])
  else
    if str = ""
    then []
      ...
      (let rec split' str ~by  accum curr =
         ...
         then (curr ^ str) :: accum
           ...
           if (String.sub str 0 lengthBy) = by
           then
             split' (String.sub str lengthBy (lengthStr - lengthBy)) ~by
               (curr :: accum) ""
           else
             split' (String.sub str 1 (lengthStr - 1)) ~by accum
               (curr ^ (String.sub str 0 1)) in
...
let linesOfChannelExn chan =
  let lines = ref [] in
  try while true do lines := ((input_line chan) :: (!lines)) done; !lines
...
let get_match_n_maybe n pat str =
  let rex = Re_pcre.regexp pat in
  try Some (Re_pcre.get_substring (Re_pcre.exec ~rex str) n) with | _ -> None
let execMaybe pat str =
  let rex = Re_pcre.regexp pat in
  try Some (Re_pcre.exec ~rex str) with | Not_found  -> None
let getSubstringMaybe result n =
  try Some (Re_pcre.get_substring result n) with | Not_found  -> None
let split sep str = let rex = Re_pcre.regexp sep in Re_pcre.split ~rex str
let rec splitInto ~chunckSize  (l : 'a list) =
  (if ((List.length l) <= chunckSize) || (chunckSize = 0)
   then [l]
   else (listTake chunckSize l) ::
     (splitInto ~chunckSize (listDrop chunckSize l)) : 'a list list)
